 ...
 * Page shows:
 * - h2 with "$XX.XX" (last month) and "$XXX.XX" (total)
 * - List of stocks on loan: links like "TICKCompany Name1 share"
 */
export async function scrapeStockLending(
  ...
  try {
    ...
    for (const text of stockLinks) {
      // Format: "TICKCompany1 share" or "ABCDOther Company10 shares"
